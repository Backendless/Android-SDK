...
import com.backendless.media.lib.rtp.RtpSocket;
...
public class RtspClient
{
  ...
  public RtspClient()
  {
    ...
    new HandlerThread( "com.backendless.media.lib.backendless.RtspClient" )
    {
      @Override
  ...
  private void sendRequestSetup() throws IllegalStateException, SocketException, IOException
  {
    for( int i = 0; i < 2; i++ )
    {
      ...
      if( stream != null )
      {
        String params = mParameters.transport == TRANSPORT_TCP ? ("TCP;interleaved=" + 2 * i + "-" + (2 * i + 1)) : ("UDP;unicast;client_port=" + (5000 + 2 * i) + "-" + (5000 + 2 * i + 1) + ";mode=receive");
  ...
  static class Response
  {
    ...
    public static Response parseResponse(
            BufferedReader input ) throws IOException, IllegalStateException, SocketException
    {

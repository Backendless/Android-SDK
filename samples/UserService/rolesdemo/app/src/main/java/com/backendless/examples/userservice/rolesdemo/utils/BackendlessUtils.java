...
public class BackendlessUtils
{
                             ...
                             final Intent intent )
  {
    login( userId, password, new BackendlessCallback<Boolean>()
    {
      ...
      public void handleResponse( Boolean loggedIn )
      {
        if( loggedIn )
        {
          unassignRole( userId, Defaults.READ_WRITE_ROLE, new BackendlessCallback<Boolean>()
          {
            @Override
            public void handleResponse( Boolean response )
            {
              unassignRole( userId, Defaults.READ_ROLE, new BackendlessCallback<Boolean>()
              {
                @Override
                public void handleResponse( Boolean response )
                {
          ...
          register( userId, password, new BackendlessCallback<Boolean>()
          {
            ...
            public void handleResponse( Boolean aBoolean )
            {
              assignRole( userId, role, new BackendlessCallback<Boolean>()
              {
                ...
                public void handleResponse( Boolean aBoolean )
                {
                  signIn( userId, password, role, viewForProgress, intent );
                }
              } );
            }
          } );
  ...
  private static void unassignRole( String userId, String role, final AsyncCallback<Boolean> callback )
  {
    Backendless.UserService.unassignRole( userId, role, new BackendlessCallback<Void>()
    {
      @Override
      public void handleResponse( Void aVoid )
      {
  ...
  public static void getAllTasks( final AsyncCallback<List<Task>> responder )
  {
    Backendless.Persistence.of( Task.class ).find( new BackendlessCallback<List<Task>>()
    {
      @Override
      public void handleResponse( List<Task> tasks )
      {
        responder.handleResponse( tasks );
      }
    } );
  }
